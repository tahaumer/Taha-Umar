...
const Button = ({ text, icon, redirectUrl, downloadUrl,invert=false, className="bg-secondary text-primary border-white/20 rounded-full" }) => {
    const buttonContent = (
        <div className='group/button flex items-center w-full'>
            <button className={`relative xl:w-[300px] lg:w-[200px] sm:w-[300px] w-full h-[50px] inline-flex items-center justify-center overflow-hidden backdrop-blur-lg px-6 py-2 text-base font-semibold ${className} transition-all duration-300 ease-in-out border`}>
            ...
            <button className={`ml-3 ${className} min-w-[50px] h-[50px] flex justify-center items-center z-10`}>
    ...
    return (
        <div className='flex justify-end w-full'>
            {downloadUrl ? (
                <a href={downloadUrl} download className='flex w-full'>
                    {buttonContent}
                </a>
            ) : redirectUrl ? (
                <Link to={redirectUrl} className='flex w-full'>
                    {buttonContent}
                </Link>
            ) : (
                buttonContent
